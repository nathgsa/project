...
const EWTCalculator: React.FC = () => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-stretch p-0 font-inter">
      ...
      <div className="w-full bg-gradient-to-br from-white to-gray-200 shadow-lg rounded-2xl p-4 sm:p-6 md:p-8">
        ...
        <form onSubmit={handleCalculate} className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 sm:pt-4">
            <button
              type="submit"
              className="w-full bg-blue-700 hover:bg-blue-600 text-white font-semibold py-3 sm:py-4 rounded-lg text-base sm:text-lg transition"
            >
              Calculate All
            </button>
            <button
      ...
      {/* Results */}
      {showResults && (
        <div className="mt-4 md:mt-8 w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 px-2 sm:px-0">
          {/* VATable */}
